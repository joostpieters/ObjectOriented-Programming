...
public abstract class Expression<T> implements Cloneable {
	...
	public Expression<T> parentExpression;
	public void setParent(Expression<T> newParentExpression)
	{
		this.parentExpression = newParentExpression;
	}
	public Statement holder;
	public void setHolder(Statement newHolder)
	{
...
//	public abstract T getValue();
	 ...
	 *       |   then result == true
	 */
	public boolean isIdenticalTo(Expression<T> other) {
		if (other == null)
			return false;
	...
	public Expression<T> clone() {
		try {
			if (isMutable())
				return (Expression<T>) super.clone();
			else
				return this;
