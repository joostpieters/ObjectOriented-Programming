 ...
 *  
 *  @invar 	Each worm has a valid name
 *  		|isValidName(getName()))
 *  @invar	Each worm has proper weapons
 *  		|hasProperWeapons()
 *  @invar	Each worm has a proper state
 *  		|hasProperState()
 * 		
 */
...
public class Worm extends Entity{
	...
	/**
	 * Check whether this worm has a program loaded
	 * 
	 ...
	 * 
	 * @post	The new state for this worm is equal to the given state
	 * 			|new.getState() == s
	 * 
	 */
	public void setState(State s)
	{
		if(canHaveAsState(s)) this.state = s;
	}
	...
	public boolean canMove() throws IllegalArgumentException
	{
		...
		Vector optimalMove;
		if(possibleMoves instanceof PriorityQueue<?>) optimalMove = (Vector)((PriorityQueue<Vector>) possibleMoves).poll();
		else optimalMove = (Vector)((ArrayList<Vector>) possibleMoves).get(0);
	...
	/**
	 * Check whether the given collection contains an optimal move 
	 * 
	 * @param 	possibleMoves
	 * 			The collection to check
	 * 
	 * @return	|Result == !possibleMoves.isEmpty()
	 * 
	 */
	...
	/**
	 * Check whether this worm can still move with the remaining action points, without taking in account a possibility of no place to move
	 * 
	 * @return	|Result == !hasOptimalMov(getPossibleMoves)) ||
	 * 			|		   getActionPoints() > costMove(optimalMove.getS())
	 * 
	 */
...

	{
		...
		Vector maxVector = (Vector)((PriorityQueue<Vector>) possibleMoves).poll();
		if(Util.fuzzyGreaterThanOrEqualTo(this.getActionPoints(), costMove(maxVector.getS())))
		{
		...
		else throw new IllegalStateException("Not enought action points");
		
		if(!world.isInRange(getRealX(), getRealY())) terminate();
	}

	/**
	 * Check whether this worm can fall
	 * 
	 * @return	|Result == getState() != State.ADJACENT
	 * 
	 */
	public boolean canFall()
	{
		return getState() != State.ADJACENT;
	}
	...
	/**
	 * Check whether this worm can shoot
	 * 
	 * @param 	yield
	 * 			The propulsion of a potential shoot
	 * 
	 * @return	Return if this worm was enough action points to shoot depending on the selected weapon and yield
	 * 			|Result == getActionPoints() >= getSelectedWeapon().getActionPointsCos(yield)
	 * 
	 */
	...
	private Collection<Vector> getPossibleMoves()
	{
		...
		//		double iAngle;
		//		
		//		for(; Util.fuzzyLessThanOrEqualTo(iRadius, realRadius); iRadius+=0.05*realRadius)
		//		{
		//			addAngle = (Math.PI/4.0)/iRadius;
		//			iAngle = 0;
		//
		//			for(; Util.fuzzyLessThan(iAngle, orientation + (Math.PI)/4); iAngle+= addAngle)
		//			{
		//				for(int index = 0; index < 2; index++ )
		//				{
		//					if(Util.fuzzyEquals(0, iAngle)) iAngle = - iAngle;
		//					else index++;
		...
		double refX = getRealX() - getRealRadius();
		double refY = getRealY() - getRealRadius();

		double start = System.currentTimeMillis();
		for(double j = refY; j <= refY + 2*realRadius; j = j+ 1)
		{
			for(double i = refX; i <= refX + 2*realRadius; i = i + 1)
			{
				if( world.isInRange(i, j) && isInSector(new Vector(i, j)))
				{

					if (!world.isImpassable(i, j, getRealRadius()))
					{
						if(world.isAdjacent(i, j, getRealRadius()))
						{
							Vector addVector = new Vector(i, j);
	 ...
	 * 
	 *@effect 	if(isInRange(getX(), getY()) isAdjacent(new.getX(), new.getY(), new.getRadius())
	 *			else isTerminated()
	 *
	 */
	public void fall()
	{
		for(double j = getRealY()+.05*getRealRadius(); j <= world.getHeight()*world.getScale(); j++)
		{
	...
	/**
	 * Set the mass for this worm
	 * 
	 * @post	new.getMass() == DENSITY * (4.0/3.0)*Math.PI*Math.pow(getRadius(),3)
	 * 
	 */
	...
	/**
	 * Set the force for this worm
	 * 
	 * @post	new.getForce() == 5*getActionPoints() + getMass()*Constants.GRAVITATION
	 * 
	 */
	 ...
	 * 
	 * @return 	|Result == world != null && world.hasAsWorm(this) && world.canHaveAsWorm(this)
	 * 
	 */
	@Override
	public boolean hasProperWorld()
	{
		return world != null && world.hasAsWorm(this) && world.canHaveAsWorm(this);
	}

	/**
	 * Check whether this worm has a proper state
	 * 
	 * @return	|Result == for each state in State.values()
	...
	public boolean hasProperState()
	{
		for(State status : State.values())
			if(status == getState()) return true;
		return false;
	}

	/**
	 * Check whether this worm can have the given state as state
	 * 
	 * @param 	state
	 * 			The new state to check
	 * 
	 * @return	|Result == for each status in State.values()
